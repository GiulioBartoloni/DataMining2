...
\paragraph{Model and tuning.} A shapelet is a subsequence maximally discriminative for a class; the Shapelet Transform represents each series by its distances to a set of retrieved shapelets, after which any standard classifier can be trained. We retrieve shapelets with a \texttt{RandomShapeletTransform} (information-gain scored) and classify the transformed features with a balanced Random Forest. A subtlety here proved important: aeon's default samples shapelet lengths \emph{uniformly up to the full $200$-step series}, so the average retrieved shapelet spans $\sim$100 steps --- half a day --- and a whole-day subsequence is a \emph{subject signature}, not a transferable motif. We therefore constrain the length to $[8,40]$ steps (local motifs of roughly one to five hours). As a fast, modern counterpart we also run \texttt{RDST} (Random Dilated Shapelet Transform), which natively uses short dilated shapelets across channels.

\paragraph{Results and what the shapes reveal.} RDST is competitive (AUC $0.620$, balanced accuracy $0.557$), but the interpretable Shapelet-Transform-plus-Forest pipeline is the most instructive result of the study. It attains a non-trivial ranking (AUC $0.593$) yet a problematic-class F1 of essentially zero at the default threshold --- the mean-pooling of a child's day-probabilities compresses them toward the prevalence, so few subjects cross $0.5$. The shapelet shapes themselves are revealing in two stages. \emph{With the length unconstrained}, the $20$ highest information-gain shapelets were whole-day shapes ($\sim$100 steps) drawn from only \emph{two} source subjects ($16$ from a single child), \emph{all} on the \texttt{enmo} channel --- a textbook \textbf{subject fingerprint masquerading as a class shapelet}: the transform had latched onto one prolific child's idiosyncratic activity-intensity signature rather than a generalizable pattern of problematic use. Constraining the length to $[8,40]$ steps (mean retrieved length $25.5$) fixes the channel pathology --- the top shapelets now spread across all five channels (\texttt{enmo}, \texttt{X}, \texttt{anglez}, \texttt{Z}, \texttt{Y}) and represent genuine local motifs --- but the top shapelets still concentrate in $\sim$two subjects and carry negligible information gain ($\approx 0.01$), and the subject-level F1 does not recover. The lesson is twofold: shapelet \emph{length} must be bounded or the method silently learns subject fingerprints (the grouped protocol and the cap are what expose this), and even well-formed short shapelets find only a weak, weakly-transferable signal in these daily recordings. RDST's short dilated shapelets, which avoid the length trap by construction, are correspondingly the better-behaved shapelet method.

\begin{figure}[h]
    \centering
    \imgph{Top: the six highest information-gain shapelets (length-constrained to $[8,40]$), colored by the class of their source series and labelled by channel/length. Bottom: the single best shapelet localized on its source series. With the length bounded the top shapelets are short, local motifs spread across all five channels (rather than the whole-day \texttt{enmo} fingerprints retrieved when length is unconstrained).}
    \caption{Retrieved short shapelets across channels (length-constrained).}
...
Shapelet Transform + RF & -- & 0.044 & 0.593 & 0.507 \\
...
Shapelet Transform + RF & len $\in[8,40]$, max\_shapelets=60 \\
...
\paragraph{Ordering and headline metric.} Because mean-pooling compresses probabilities, F1 at a fixed $0.5$ threshold is fragile (most visibly for Shapelet-Transform-plus-Forest, which ranks well by AUC yet predicts almost no positives); we therefore read the ranking primarily off AUC and balanced accuracy. On those, the order is clear: \textbf{MiniRocket} $(0.645)$ $>$ \textbf{RDST} $(0.620)$ $>$ \textbf{Shapelet} $(0.593) \approx$ \textbf{MUSE} $(0.590)$ $>$ \textbf{KNN-DTW} $(0.542)$ $>$ Manhattan $>$ Euclidean, with the dummy at $0.500$. The story mirrors the benchmark literature and the report's tabular narrative: a cheap distance baseline that barely separates the classes, interpretable shapelet methods that expose a data pathology (and recover only a weak signal once that pathology is controlled), and a fast convolutional kernel method that is the strongest learner.
